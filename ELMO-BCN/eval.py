...
# parser.add_argument("--datadir", type=str, default='/cache/datasets',
#                     help="MedelArts:Path to the directory that contains the datasets")
parser.add_argument("--outputdir", type=str, default='model', help="Path to the directory where the BCN model will be saved")
# parser.add_argument("--outputdir", type=str, default='/cache/result',
#                     help="MedelArts:Path to the directory where the BCN model will be saved")

parser.add_argument("--mode", type=int, default=0,
                    help="0: Normal (train + test); 1: BCN model dry-run (just try creating the model and do nothing else); 2: Train + test dry-run (Load a smaller dataset and train + test on it)")

parser.add_argument("--transfer_task", type=str, default="SSTFine",
                    help="Transfer task used for training BCN and evaluating predictions (e.g. SSTBinary, SSTFine, SSTBinary_lower, SSTFine_lower, TREC6, TREC50, TREC6_lower, TREC50_lower)")

parser.add_argument("--n_epochs", type=int, default=20,
                    help="Number of epochs (int). After 5 epochs of worse dev accuracy, training will early stopped and the best epoch will be saved (based on dev accuracy).")
parser.add_argument("--batch_size", type=int, default=32, help="Batch size (int)")
parser.add_argument("--same_bilstm_for_encoder", type=str, default="True",
                    help="Whether or not to use the same BiLSTM (when flag is set) or separate BiLSTMs (flag unset) for the encoder (str: True or False)")
parser.add_argument("--bilstm_encoder_n_hidden", type=int, default=300,
                    help="Number of hidden states in encoder's BiLSTM(s) (int)")
parser.add_argument("--bilstm_encoder_forget_bias", type=float, default=1.0,
                    help="Forget bias for encoder's BiLSTM(s) (float)")
parser.add_argument("--bilstm_integrate_n_hidden", type=int, default=300,
                    help="Number of hidden states in integrate's BiLSTMs (int)")
parser.add_argument("--bilstm_integrate_forget_bias", type=float, default=1.0,
                    help="Forget bias for integrate's BiLSTMs (float)")
parser.add_argument("--dropout_ratio", type=float, default=0.1,
                    help="Ratio for dropout applied before Feedforward Network and before each Batch Norm (float)")
parser.add_argument("--output_reduction", type=int, default=2,
                    help="On the first and second maxout layers, the dimensionality is divided by this number (int)")
...
parser.add_argument("--adam_beta1", type=float, default=0.9,
                    help="Beta1 for adam optimiser if adam optimiser is used (float)")
parser.add_argument("--adam_beta2", type=float, default=0.999,
                    help="Beta2 for adam optimiser if adam optimiser is used (float)")
parser.add_argument("--adam_epsilon", type=float, default=1e-8,
                    help="Epsilon for adam optimiser if adam optimiser is used (float)")
...
# for learning_rate in [0.0001, 0.0003, 0.006, 0.001, 0.003]:
...
# accuracy = {'learning_rate': learning_rate, 'dev_accuracy': dev_accuracy, 'test_accuracy': test_accuracy}
accuracy = {'dev_accuracy': dev_accuracy, 'test_accuracy': test_accuracy}
with open(os.path.join(args.outputdir, "accuracy.txt"), "a") as outputfile:
