...
class ELMOBCN:
    def __init__(self, params, n_classes, max_sent_len, outputdir, weight_init=0.01, bias_init=0.01,model_dir=None):
        self.params = params
    ...
    def create_model(self):
        ...
        # elmo = hub.Module("/cache/datasets/elmo-model/elmo-bcn", trainable=True)
        elmo = hub.Module(r"C:\Users\Lenovo\AppData\Local\Temp\tfhub_modules\9bb74bc86f9caffc8c47dd7b33ec4bb354d9602d", trainable=True)
        ...
        with tf.variable_scope("output"):
            ...
            #                                            global_,
            #                                            decay_steps=self.params['n_epochs'],
            #                                            decay_rate=0.3)
            #  =============设置动态学习率=============== #
    ...
    def dry_run(self,model_dir=None):
        tf.reset_default_graph()
        with tf.Graph().as_default():
            return self.create_model(model_dir=model_dir)

    def train(self, dataset, model_dir=None):
        best_dev_accuracy = -1
        tf.reset_default_graph()
        with tf.Graph().as_default() as graph:
            textinputs1, inputs1length, textinputs2, inputs2length, labels, is_training, predict, loss_op, train_op = self.create_model()
            # textinputs1, inputs1length, textinputs2, inputs2length, labels, is_training, predict, loss_op, train_op,current_epoch,lr = self.create_model()
        with tf.Session(graph=graph) as sess:
            ...
            for epoch in range(self.params['n_epochs']):
                ...
                X1_iterator, X1length_iterator, X2_iterator, X2length_iterator, y_iterator = dataset.get_data_iterator('train', self.params['batch_size'])
                sess.run(X1_iterator.initializer)
                sess.run(X1length_iterator.initializer)
                sess.run(X2_iterator.initializer)
                sess.run(X2length_iterator.initializer)
                sess.run(y_iterator.initializer)
                for i in range(total_train_batches):
                    ...
                    # try:
                    #     batch_X1 = sess.run(X1_iterator.get_next())
                    #     batchX1length = sess.run(X1length_iterator.get_next())
                    #     batch_X2 = sess.run(X2_iterator.get_next())
                    #     batch_X2length = sess.run(X2length_iterator.get_next())
                    #     batch_y = sess.run(y_iterator.get_next())
                    #     # print(result)
    ...
    def calculate_accuracy(self, dataset, sess, textinputs1, inputs1length, textinputs2, inputs2length, labels, is_training, predict, set_name="test", verbose=False):
        ...
        X1_iterator, X1length_iterator, X2_iterator, X2length_iterator, y_iterator = dataset.get_data_iterator(set_name,self.params['batch_size'])
        sess.run(X1_iterator.initializer)
        sess.run(X1length_iterator.initializer)
        sess.run(X2_iterator.initializer)
        sess.run(X2length_iterator.initializer)
    ...
    def test(self, dataset,model_dir=None):
        tf.reset_default_graph()
        with tf.Graph().as_default() as graph:
