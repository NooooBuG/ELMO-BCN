...
class SSTDataset(object):
    ...
    def get_data(self, textual_data):
        ...
        for key in textual_data:
            ...
            for tokenized_sentence in textual_data[key]['X']:
                ...
                data[key]['X1'].append(self.pad(tokenized_sentence))  # 补全空格，使tokenized_sentence长度相同
                done += 1
            ...
            data[key]['X2length'] = data[key][
                'X1length']  # Only one input sentence is needed for SSTBinary so X1 is duplicated
    ...
    def get_data_iterator(self, key, batch_size):
        # 构造 tf.dataset
        X1 = tf.data.Dataset.from_tensor_slices((self.data[key]['X1']))
        X1length = tf.data.Dataset.from_tensor_slices((self.data[key]['X1length']))
        X2 = tf.data.Dataset.from_tensor_slices((self.data[key]['X2']))
        X2length = tf.data.Dataset.from_tensor_slices((self.data[key]['X2length']))
        y = tf.data.Dataset.from_tensor_slices((self.data[key]['y']))
        # tensorflow中去除不足一个batch的数据
        X1 = X1.apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        X1length = X1length.apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        X2 = X2.apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        X2length = X2length.apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        y = y.apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        # 构造数据迭代器
        X1_iterator = X1.make_initializable_iterator()
        X1length_iterator = X1length.make_initializable_iterator()
        X2_iterator = X2.make_initializable_iterator()
        X2length_iterator = X2length.make_initializable_iterator()
        y_iterator = y.make_initializable_iterator()

        return X1_iterator, X1length_iterator, X2_iterator, X2length_iterator, y_iterator
